                    ...
                    Left _-> let    (Left st)=x
                                    ...
                                    failMsg=Right ("Type mismatch. Expect:  "++(show st)++"  Got:  "++(show tt)++"\n")
                                        in case matchTypeH [(0,[],[])] st tt of
                                                Right _->failMsg
                                                Left (returnT,as)->case returnT of
                                                                        Nothing->Left st
                                                                        Just t->case (concatType t as) of 
                                                                                    Just t'->Left $ setReturnType st t'
                                                                                    Nothing->failMsg
                                                                        
                                                                         

matchTypeH :: [(TagC,[(Tag,Type)],[(Tag,Type)])]->Type->Type->Either (Maybe Type,[(TagC,[(Tag,Type)],[(Tag,Type)])]) ()
    ...
    = let   (tc',stv')=addPH tc stv tye1
            (tc'',ttv')=addPH tc' ttv tye2
                in case matchTypeH ((tc''+1,[],[]):(tc'',stv',ttv'):as) t11 t21 of
                            Left (_,(a:as))->case t12 of 
                                                    UnknownRe->Left (Just t22,as) --Provide return value type calculation
                                                    _->matchTypeH as t12 t22
                            Right _->Right ()
matchTypeH a@((tc,stv,ttv):as) t1@(SingleType tag1 (TypeENV _ tye1)) t2@(SingleType tag2 (TypeENV _ tye2))
    |isTemp tag1 && isTemp tag2 = addN $ setTmpVarType a' tag2 (getTmpVarTypeL a' tag1)
    |isTemp tag1 = Right ()
    |isTemp tag2 = addN $ setTmpVarType a' tag2 (getTmpVarTypeL a' tag1)
    |otherwise = if tag1 == tag2 then Left (Nothing,a) else Right ()
...
matchTypeH ((tc,stv,ttv):as) t1 t2@(SingleType tag2 (TypeENV _ tyenv)) = let    (tc',ttv')=addPH tc ttv tyenv
                                                                                a'=((tc',stv,ttv'):as)
                                                                                    in addN $ setTmpVarType a' tag2 t1
...
setReturnType (SingleType _ _) t = t
setReturnType (ArrowType e t1 t2) t = ArrowType e t1 (setReturnType t2 t)
